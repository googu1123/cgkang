...
<meta charset="utf-8">
	...
	<script src="${pageContext.request.contextPath}/js/paging.js"></script>
 	...
 	var boardMain = {
 		...
 		,btnEvent : function(){
		...
		},getPartneristerList :function(no){
    		...
    		$.ajax({ 
    			...
    			success : function(resData){
    				...
    				var item = JSON.parse(resData.partnerlist);
					var selectHtml=[]; 
					var len = item.length;
					
					var totalCount = resData.totalCount;
    				var page_partnerList = Paging(totalCount, LIMIT, PAGEGROUPCNT ,pageNo, "partnerList"); //공통 페이징 처리 함수 호출
    				...
    				if(len > 0){
    					...
    					for (var i = 0; i < item.length; i++){
        					...
        					selectHtml.push('<tr style="height:40px;" class="seq_tr" id="'+item[i].seq+'">'); 
	    						...
	    						selectHtml.push('<td>'+item[i].rank+'</td>');
	    						selectHtml.push('<td>'+item[i].phone+'</td>');
    				...
    				//페이징 그리기 
    				$("#paging").empty().html(page_partnerList);
 	...
 	var goPaging_partnerList = function(cPage){
    	boardMain.getPartneristerList(cPage); 
    };
